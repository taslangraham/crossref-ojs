 ...
 *
 * @brief Handle API request for Crossref's Citedby
 */
...
class CrossrefCitedByController extends PKPBaseController
{
    ...
    /**
     * Get all crossref citations for an article
     */
    public function getCitations(Request $illuminateRequest): JsonResponse
    {
        ...
        if (!$enabledRegistrationAgency instanceof CrossrefPlugin || !$enabledRegistrationAgency->getSetting($context->getId(), 'citedBy')) {
            return response()->json([
                'error' => __('plugins.generic.crossref.api.citedByNotEnabled')
            ], Response::HTTP_FORBIDDEN);
        }
Cache::delete("crossref-citedBy-{$submissionId}"); // Clear the cache for testing purposes
        try {
     ...
     */
    protected function getCitedByCacheMiss(Submission $submission, $plugin, $context): array
    {
        ...
        $results = [];
        $dois = ['10.12688/openreseurope.13277.2', '10.12688/openreseurope.13277.1'];
            ...
            foreach (CrossrefCitedBy::citationTypes() as $citeType) {
                if ($item->{$citeType}) {
                    if ($citeType === 'msg') {
                        ...
                        $result = [...$result, ...$this->extractCitationsFromXMLList($item->{$citeType}->forward_link)];
                    } else {
                        $result[] = $this->getCitationData($item, $citeType);
                    }
                    break;
                }
            }
    ...
    private function extractAuthorList(SimpleXMLElement $item, string $type): string
    {
        ...
        foreach ($contributors as $contributor) {
            $name = trim($contributor->surname . ' ' . $contributor->given_name);
